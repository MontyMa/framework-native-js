<template>
	<div class="register_7">
		<p :class="{active: thisvalidationMobile}">绑定手机</p>
		<span v-if="!thisvalidationMobile">您尚未绑定手机，为了您的交易，资金安全，建议您绑定手机</span>
		<span v-if="thisvalidationMobile">您已绑定手机</span>
		<div class="circle_box" :class="{active : thisvalidationMobile}">
			<div class="circle_shadow"></div>
			<div class="circle"></div>
		</div>
		<a href="javascript:;" v-if="!thisvalidationMobile" @click="goCheck">立即认证</a>
		<i class="checked_icon" v-if="thisvalidationMobile"></i>
		<div class="bar"></div>
	</div>
</template>
