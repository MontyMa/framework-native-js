...
<script>
  ...
  export default{
    ...
    methods: {
      ...
      //发起请求
      getRequest(current){
        this.getLegitimateData.param.pageNo = current;     //拼合参数

        lh_http('post', this.getLegitimateData.url, this.getLegitimateData.param).done(response => {
          this.getPagingNoList(current, response.data.list.totalPages + 1);
