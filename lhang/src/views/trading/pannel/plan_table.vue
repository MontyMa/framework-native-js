<template>
  <div class="common_table" id="common_table">
    <!--<div class="batch_btn" v-if="tradeType === '1'"-->
    <!--@click="revokedOrder(batch_revoked_data,4,list_r.triggerPrice)"-->
    ...
    <!--<td v-if="tradeType === '1'" style="text-align: center" colspan="8">暂无数据</td>-->
    <!--<td v-else="tradeType === '1'" style="text-align: center" colspan="7">暂无数据</td>-->
    <!--</tr>-->
    <!--<tr v-for="item in list_r" v-else :class="{layout:tradeType==='2'}" class="tbody_tr">-->
    <!--<td class="spe_title">-->
    <!--<input type="checkbox" v-if="tradeType === '1'"-->
    <!--v-model="checked"-->
    <!--:value="{uuid:item.uuid,triggerPrice:item.triggerPrice}"/>-->
    <!--<span v-if="tradeType==='1'">{{item._time}}</span>-->
    <!--<span v-else="tradeType==='1'">{{item.spotOrderTime}}</span>-->
    <!--</td>-->
    <!--<td>计划委托</td>-->
    <!--<td>{{item.amount}}</td>-->
    <!--<td>{{item.price}}</td>-->
    <!--<td>{{item.triggerPrice}}</td>-->
    <!--<td v-if="tradeType ==='1'">{{item.surplusAmount}}</td>-->
    <!--<td :style="{color:item.orderStatus!==0?'#494949':'#E93C6F'}">{{item._status_txt}}</td>-->

    <!--<td v-if="tradeType === '1'">-->
    <!--<div class="revoked" @click="revokedOrder(item.uuid,1,item.triggerPrice)">-->
    <!--<span>撤单</span>-->
    <!--</div>-->
    <!--</td>-->
    ...
    <!--<page v-if="!empty"-->
    <!--:totalPages='paging_data.pageTotal'-->
...
<!--return {-->
<!--uuid: elem.uuid,-->
<!--triggerPrice: elem.triggerPrice-->
<!--};-->
<!--})-->
<!--} else {-->
<!--this.checked = [];-->
...
<!--lh_http("post", "order/listPlan", this.list_p, page).done((res) => {-->
<!--if (!res.status) {-->
<!--alert(res.data);-->
<!--return;-->
<!--}-->
...
<!--this.list_p.useruuid = this.uuid;-->
<!--this.list_p.category = this.$route.params.type + "_cny";-->
<!--lh_http("post", "order/listPlanHistroy", this.list_p, page).done((res) => {-->
<!--if (!res.status) {-->
<!--alert(res.data);-->
<!--return;-->
<!--}-->
